...
".config-input{background:#1F2937;border:1px solid #374151;border-radius:0.375rem;padding:0.5rem;color:#F9FAFB;width:70px;text-align:center;}"
".config-input:focus{outline:none;border-color:#4F46E5;}"
".config-btn{background:#4F46E5;color:white;border:none;padding:0.5rem 1rem;border-radius:0.375rem;cursor:pointer;transition:background-color 0.2s;}"
...
"})"
"};"
"let state={maxLimit:70,minLimit:10,userEditing:false};"
"const chartOptions={"
...
"if(minTemp>=maxTemp){"
"alert('Temperatura mínima deve ser menor que máxima!');"
"return;"
"}"
"state.minLimit=minTemp;"
"state.maxLimit=maxTemp;"
...
"if(!state.userEditing){"
"if(d.maxTemperature!==undefined){"
"state.maxLimit=d.maxTemperature;"
"document.getElementById('max-temp').value=d.maxTemperature;"
"}"
"if(d.minTemperature!==undefined){"
"state.minLimit=d.minTemperature;"
"document.getElementById('min-temp').value=d.minTemperature;"
"}"
"}"
