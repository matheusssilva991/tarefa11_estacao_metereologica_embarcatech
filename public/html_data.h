...
"chart.render();"
"function saveLimits(){const minTemp=parseInt(document.getElementById('min-temp').value),maxTemp=parseInt(document.getElementById('max-temp').value);if(minTemp>=maxTemp){alert('Temperatura mínima deve ser menor que máxima!');return}state.minLimit=minTemp;state.maxLimit=maxTemp;state.userEditing=false;fetch('/api/limits',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({min:minTemp,max:maxTemp})}).then(r=>r.text()).then(data=>{alert('Limites salvos!');console.log('Resposta:',data)}).catch(e=>{alert('Erro ao salvar!');console.error('Erro:',e);state.userEditing=false})}"
...
"async function updateData(){try{const r=await fetch('/api/weather'),d=await r.json(),newTemp=d.temperature,newHumidity=d.humidity,newPressure=d.pressure,newAltitude=d.altitude;if(!state.userEditing){if(d.maxTemperature!==undefined){state.maxLimit=d.maxTemperature;document.getElementById('max-temp').value=d.maxTemperature}if(d.minTemperature!==undefined){state.minLimit=d.minTemperature;document.getElementById('min-temp').value=d.minTemperature}}document.getElementById('temp-value').textContent=newTemp+' °C';document.getElementById('humidity-value').textContent=Math.round(newHumidity)+' %';document.getElementById('pressure-value').textContent=Math.round(newPressure)+' hPa';document.getElementById('altitude-value').textContent=Math.round(newAltitude)+' m';const now=new Date();document.getElementById('last-update').innerHTML='<i class=\"fas fa-clock text-blue-400\"></i> Última atualização: '+now.toLocaleTimeString('pt-BR');chartData.temp.push(newTemp);chartData.humidity.push(newHumidity);chartData.pressure.push(newPressure);chartData.categories.push(now.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit',second:'2-digit'}));if(chartData.temp.length>20){chartData.temp.shift();chartData.humidity.shift();chartData.pressure.shift();chartData.categories.shift()}updateChartSeries()}catch(e){console.error('Erro:',e)}}"
